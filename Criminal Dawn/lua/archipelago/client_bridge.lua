...
function apd2_poll_client()
  ...
  -- Abort file read if the seed doesn't match the client
  if apd2_data.game.seed ~= APD2Client.seed then
    log(APD2FileIdent .. "Seed mismatch - I ain't reading all that")
    return
  end
  ...
  if APD2Client["Drill Sawgeant"] ~= 0 then
    ...
    if FoundUpgrade ~= true then
      log(APD2FileIdent .. "Drill speed " .. APD2Client["Drill Sawgeant"])
  ...
  if APD2Client["Nine Lives"] ~= 0 then
    ...
    if FoundUpgrade ~= true then
      log(APD2FileIdent .. "Nine Lives " .. APD2Client["Nine Lives"])
  ...
  -- Enable One Down
  if not apd2_data.x.od == 2 and APD2Client["One Down"] then
...
-- Show host's PONR time remaining
